...
namespace IHI.Server.Useful
{
    public class WeakList<T> : IList<T> where T : class
    {
        ...
        private readonly List<WeakReference<T>> _internalCollection;
        #endregion
        #endregion

        #region Methods
        #region Method: WeakList (Constructor)
        public WeakList()
        {
            _internalCollection = new List<WeakReference<T>>();
        }
        #endregion
        #endregion
        ...
        public void Add(T item)
        {
            ...
            int existingIndex = _internalCollection.IndexOf(weakItem);
            if (existingIndex >= 0)
            {
                if (!_internalCollection[existingIndex].IsAlive)
                    _internalCollection.RemoveAt(existingIndex);
            }
            _internalCollection.Add(weakItem);
        }
        ...
        {
            _internalCollection.Insert(index, new WeakReference<T>(item));
        }
        ...
        public T this[int index]
        {
            get
            {
                WeakReference<T> weakValue = _internalCollection[index];
                if (!weakValue.IsAlive)
                {
                    _internalCollection.RemoveAt(index);
                    return _internalCollection[index].Target;
                }
                return weakValue.Target;
            }
            set
            {
                _internalCollection[index] = new WeakReference<T>(value);
            }
        }
    }
}